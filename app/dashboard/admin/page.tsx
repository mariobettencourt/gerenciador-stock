...
export default function AdminInventario() {
  ...
  const [emailAdmin, setEmailAdmin] = useState("Desconhecido"); // Guardar quem faz a ação
  ...
  // Estados de Criação (O que tinha desaparecido)
  ...
  useEffect(() => { 
    carregarProdutos(); 
    // Vai buscar o email de quem está ligado
    supabase.auth.getUser().then(({ data }) => {
      if (data.user?.email) setEmailAdmin(data.user.email);
    });
  }, []);
  ...
  const apagarArtigo = async (id: number, nome: string) => {
    if (confirm(`Tem a certeza ABSOLUTA que quer apagar "${nome}" do sistema?`)) {
      ...
      // Regista na Auditoria PRIMEIRO para guardar o nome
      await supabase.from("movimentos").insert({
        tipo: "Remoção",
        utilizador: emailAdmin,
        quantidade: 0,
