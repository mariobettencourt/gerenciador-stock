...
export default function ProcessarPedido() {
  ...
  // ID do utilizador logado para auditoria
  ...
  // --- FUNÇÃO CRÍTICA: FINALIZAR E GERAR SNAPSHOT DE PREÇO ---
  const finalizarProcessamento = async (e: React.FormEvent) => {
    ...
    try {
      for (const linha of linhas) {
        // 1. Procurar o produto no catálogo para obter o preço e stock ATUAL
        const produtoAtual = catalogo.find(p => p.id === parseInt(linha.produto_id));
        const precoNoMomento = produtoAtual?.preco || 0; 

        // 2. Registar o Movimento de Saída com SNAPSHOT do preço
        const { error: erroMovimento } = await supabase.from("movimentos").insert({
          ...
          custo_unitario: precoNoMomento // <-- CONGELA O PREÇO AQUI
        });

        if (erroMovimento) {
          alert("ERRO NA AUDITORIA: " + erroMovimento.message);
          return; 
        }

        // 3. Abater o Stock Físico na tabela de Produtos
        if (produtoAtual) {
          ...
          if (erroStock) {
            alert("ERRO AO ATUALIZAR STOCK: " + erroStock.message);
            return;
          }
        }
      }
      ...
      alert("✅ Pedido processado e stock atualizado com sucesso!");
      router.push("/dashboard/pedidos");

    } catch (err) {
      alert("Ocorreu um erro ao finalizar o processamento.");
    }
  };
    ...
    <main className="flex-1 p-12 overflow-y-auto h-screen relative bg-slate-50">
      ...
      <form onSubmit={finalizarProcessamento} className="bg-white rounded-[3rem] p-10 shadow-2xl border-4 border-white flex flex-col min-h-[500px]">
        <h2 className="text-xl font-black uppercase italic tracking-tighter mb-6 flex justify-between items-center border-b border-gray-100 pb-4 text-[#0f172a]">
          <span>Artigos na Caixa de Envio</span>
        ...
        <div className="space-y-4 mb-8 flex-1">
          {linhas.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-200 rounded-3xl bg-gray-50/50">
              ...
              <p className="text-xs font-black uppercase text-gray-400 tracking-widest">Caixa vazia.</p>
            </div>
          ) : (
            linhas.map((linha, index) => (
              <div key={index} className="flex gap-4 items-center bg-gray-50 p-4 rounded-2xl border border-gray-100">
                <div className="flex-[2]">
                  ...
                  <p className="text-[10px] text-gray-500 font-bold tracking-widest mt-1 uppercase">Local: {linha.local} | Stock: {linha.stockMax} un.</p>
                </div>
                ...
                <div className="flex-[2]">
                  <label className="text-[9px] font-black text-gray-400 uppercase block mb-1 px-1 tracking-widest">Nota na Guia</label>
                  <input type="text" value={linha.observacao} onChange={e => atualizarLinha(index, "observacao", e.target.value)} className="w-full bg-white border-2 border-slate-200 rounded-xl py-3 px-4 text-xs font-bold outline-none focus:border-blue-500" placeholder="Ex: Pack fechado..." />
                </div>
        ...
        <div className="fixed inset-0 bg-[#0f172a]/80 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <div className="bg-white rounded-[3rem] p-8 w-full max-w-4xl shadow-2xl h-[80vh] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black text-[#1e3a8a] uppercase italic tracking-tighter">Escolher Material</h2>
            ...
            <div className="mb-6 relative">
              ...
              <input 
                type="text" autoFocus placeholder="Procurar artigo..." 
